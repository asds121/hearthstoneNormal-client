  ...
  return {
      ...
      if (_status.connectMode) {
        ...
        if (_status.mode == "zhong") {
          ...
          if (lib.configOL.enable_commoner || lib.configOL.double_nei) {
            ...
            for (
              var i = 1;
              i < lib.config.mode_config.identity.identity.length;
              i++
            ) {
    ...
    game: {
      ...
      getRoomInfo: function (uiintro) {
        ...
        if (lib.configOL.identity_mode != "zhong") {
          if (lib.configOL.identity_mode == "stratagem") {
            ...
            uiintro.add(
              '<div class="text chat">双内奸：' +
                (lib.configOL.double_nei ? "开启" : "关闭")
            );
            if (lib.configOL.identity_mode != "stratagem") {
              uiintro.add(
          ...
          uiintro.add(
            '<div class="text chat">卡牌替换：' +
              (lib.configOL.zhong_card ? "开启" : "关闭")
          );
        }
          ...
          game.broadcastAll(
            function (players, identityList, list) {
              ...
              for (var i = 0; i < players.length; i++) {
                ...
                players[i].setIdentity(
                  list.includes(identityList[i]) ? "cai2" : "cai"
                );
      ...
      chooseCharacterStratagemOL: function () {
        ...
        next.setContent(function () {
          ...
          ["stratagem_gain", "stratagem_insight", "stratagem_expose"].forEach(
            (globalSkill) => game.addGlobalSkill(globalSkill)
          );
        ...
        next.setContent(function () {
          ...
          var addSetting = function (dialog) {
            ...
            for (var i = stratagemMode ? 1 : 2; i <= game.players.length; i++) {
              ...
              if (
                !stratagemMode &&
                get.distance(game.zhu, game.me, "absolute") === i - 1
              ) {
                td.classList.add("bluebg");
              }
              td.addEventListener(
          ...
          if (!event.chosen.length) {
            game.me
              ...
              .set("onfree", true).selectButton = function () {
          ...
          for (let i = 0; i < game.players.length; i++) {
            if (game.players[i] != game.zhu) {
              ...
              if (game.players[i].special_identity) {
                str +=
                  "（" +
                  get.translation(game.players[i].special_identity) +
                  "）";
              }
              list.push([
                game.players[i],
    ...
    element: {
      ...
      content: {
        stratagemInsight: () => {
          ...
          var insightResult = (event.insightResult = get.insightResult(
            player,
            target
          ));
        ...
        if (_status.mode == "purple") {
          ...
          if (
            ...
            game.players
              ...
              .map((i) => i.identity)
              .reduce((p, c) => (!p.includes(c) ? p.push(c) && p : p), [])
              .length == 1
          ) {
            return real;
          }
          ...
          if (["rZhu", "rZhong", "bNei"].includes(from.identity)) {
            ...
            if (
              ...
              game.hasPlayer(function (current) {
                return ["rZhong", "bZhong", "rNei", "bNei"].includes(
                  current.identity
                );
              })
            ) {
                ...
                case "zhu":
                  ...
                  if (get.population("fan") == 1) {
                    ...
                    if (situation > 1 || get.population("fan") == 0) {
                      ...
                      num =
                        get.population("fan") + Math.max(0, 3 - game.zhu.hp);
                    }
                  }
                  if (strategy == 2) {
                    num--;
    ...
    skill: {
      ...
      stratagem_insight: {
        ...
        logTarget: "player",
        prompt2: (event) =>
          `消耗1点怒气，洞察${get.translation(event.player)}的身份`,
        check: (event, player) => {
        ...
        subSkill: {
          cancel: {
            ...
            filter(event, player) {
              ...
              return (
                evt.name === "die" &&
                evt.player.identity === "fan" &&
                event.getParent(4).name === "sixiang_zhuque"
              );
            },
            silent: true,
            forceDie: true,
