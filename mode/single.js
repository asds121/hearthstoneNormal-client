...
export default () => {
  return {
    ...
    game: {
      ...
      getRoomInfo: function (uiintro) {
        if (lib.configOL.single_mode == "normal") {
          uiintro.add(
            '<div class="text chat">晋势力武将：' +
              (lib.configOL.enable_jin ? "开启" : "关闭")
          );
        }
                    ...
                    `<div class="text">${game.globalBuff
                      .map((buff, ind) => {
                        return (
                          ...
                          "：" +
                          get.skillInfoTranslation(buff, null, false)
                        );
                      })
                      .join("<br>")}</div>`
                  );
      ...
      chooseCharacter: function () {
        ...
        next.setContent(function () {
          ...
          ("step 9");
          game.me.init(result.links[0], null);
          _status.characterChoice[game.me.identity].removeArray(result.links);
          var list =
            _status.characterChoice[game.me.enemy.identity].randomRemove(1);
          game.me.enemy.init(list[0]);
          ...
          game.broadcastAll(
            function (list, id) {
              ...
              var dialog = ui.create
                .characterDialog("heightset", filter)
                .open();
              dialog.videoId = id;
              ui.arena.classList.add("choose-character");
            },
    ...
    translate: {
      ...
      xiechan: "挟缠",
      xiechan_info:
        "限定技，出牌阶段，你可以和对手拼点。然后赢的角色视为对没赢的角色使用一张【决斗】。",
      huwei: "虎威",
