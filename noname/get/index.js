...
export class Get extends GetCompatible {
  ...
  prompt(skill, target, player) {
    ...
    if (target) {
      ...
      return (
        "是否对" + str + "发动【" + get.skillTranslation(skill, player) + "】？"
      );
    } else {
  ...
  modetrans(config, server) {
    ...
    if (config.mode == "identity") {
      switch (config.identity_mode) {
        ...
        case "zhong":
          return "忠胆英杰";
        case "stratagem":
          return get.cnNumber(parseInt(config.number)) + "人谋攻";
        default:
          return `${get.cnNumber(parseInt(config.number))}人${config.enable_commoner ? "带民" : ""}身份`;
      }
    }
    ...
    if (server) {
      ...
      return (
        ...
        "人" +
        get.translation(get.plainText(config.mode))
      );
    }
  }
  /**
  ...
  skillRank(skill, type, grouped) {
    ...
    if (info.ai) {
      ...
      if (
        typeof info.ai.combo == "string" &&
        player &&
        !player.hasSkill(info.ai.combo)
      ) {
        return 0;
      }
      if (info.ai.neg) {
  ...
  itemtype(obj) {
    ...
    if (
      obj instanceof lib.element.GameEvent ||
      obj instanceof lib.element.GameEventPromise
    ) {
      return "event";
    }
      ...
      if (get.itemtype(str) == "card" || str.isCard) {
        ...
        if ((str.suit && str.number) || str.isCard) {
          ...
          if (arg == "viewAs" && str.viewAs != str.name && str.viewAs) {
            ...
            str2 +=
              "【" + get.translation(get.suit(str, false)) + cardnum + "】";
  ...
  *iterableChildNodes(node) {
  ...
  selectableTargets(sort) {
    ...
    var players = game.players.slice(0);
    if (
      _status.event.deadTarget ||
      (_status.event.skill && get.info(_status.event.skill)?.deadTarget) ||
      (get.card() && get.info(get.card())?.deadTarget)
    ) {
  ...
  tag(item, tag, item2, bool) {
    ...
    if (
      get.info(item, bool) &&
      get.info(item, bool).ai &&
      get.info(item, bool).ai.tag
    ) {
  ...
  nodeintro(node, simple, evt) {
    ...
    if (typeof node._customintro == "function") {
      ...
      for (i = 0; i < skills.length; i++) {
        if (
          lib.skill[skills[i]] &&
          (lib.skill[skills[i]].nopop || lib.skill[skills[i]].equipSkill)
        ) {
          continue;
        }
        if (lib.translate[skills[i] + "_info"]) {
          ...
          if (node.forbiddenSkills[skills[i]]) {
            ...
            if (node.forbiddenSkills[skills[i]].length) {
              forbidstr +=
                "（与" +
                get.translation(node.forbiddenSkills[skills[i]]) +
                "冲突）<br>";
            } else {
              forbidstr += "（双将禁用）<br>";
            }
      ...
      if (!simple || get.is.phoneLayout()) {
        ...
        for (var i = 0; i < js.length; i++) {
          ...
          if (
            ...
            uiintro.add(
              `<div><div class="skill">${html}</div><div>${lib.translate[js[i].viewAs]}：${lib.card[js[i].viewAs]?.cardPrompt?.(js[i], node) || lib.translate[`${js[i].viewAs}_info`]}</div></div>`
            );
          } else {
            uiintro.add(
                ...
                function () {
                  if (this._link) {
                    if (avatar2) {
                      ...
                      node.node.avatar2.style.backgroundImage =
                        this.style.backgroundImage;
                    } else {
                      lib.config.skin[nameskin] = this._link;
                      node.node.avatar.style.backgroundImage =
                        this.style.backgroundImage;
                    }
                  } else {
      ...
      //卡牌长按介绍
      if (
        ui.arena.classList.contains("observe") &&
        node.parentNode.classList.contains("handcards")
      ) {
        return;
      }
      var name = node.name,
      ...
      if (node._banning) {
        ...
        for (var i = 0; i < list.length; i++) {
          ...
          if (list[i] == "zhinang_tricks") {
            cfg.bannedname =
              (node._banning == "offline" ? "" : "connect_") + "zhinang_tricks";
        ...
        if (lib.translate[name + "_info"]) {
            ...
            if (lib.card[name] && lib.card[name].derivation) {
              if (typeof lib.card[name].derivation == "string") {
                ...
                uiintro.add(
                  '<div class="text center">来源：' +
                    get.translation(
                      lib.card[name].derivationpack + "_card_config"
                    ) +
                    "包</div>"
                );
              }
          ...
          if (get.is.yingbianConditional(node.link || node)) {
            ...
            if (yingbianEffects.length && showCardIntro) {
              uiintro.add(
                `<div class="text" style="font-family: yuanli">应变：${yingbianEffects.map((value) => lib.yingbian.prompt.get(value)).join("；")}</div>`
              );
            }
          }
          if (lib.translate[name + "_append"]) {
    ...
    if (typeof card != "string" && (typeof card != "object" || !card.name)) {
      ...
      if (event.skill && skillinfo.viewAs == undefined) {
        ...
        if (
          skillinfo &&
          skillinfo.viewAs &&
          card.name === skillinfo.viewAs.name
        ) {
