...
export class Game extends GameCompatible {
  ...
  setNature(item, nature, addNature) {
    ...
    if (!addNature) {
      ...
      let natures = Array.isArray(nature)
        ? nature
  ...
  washCard() {
    ...
    cards.randomSort();
    return game.cardsGotoPile(cards, "triggeronly", "washCard", [
      "shuffleNumber",
      game.shuffleNumber,
    ]);
  }
  ...
  setSimpleYingbianCondition(yingbianCondition, condition) {
    return game.broadcastAll(
      (yingbianCondition, condition) =>
        lib.yingbian.condition.simple.set(yingbianCondition, condition),
      yingbianCondition,
      condition
    );
  }
  ...
  playBackgroundMusic() {
    ...
    if (music == "music_custom") {
      const backgroundMusicSourceConfiguration =
        lib.config.background_music_src;
    ...
    if (objectConfig) {
      Object.defineProperties(
        ...
        Object.keys(objectConfig).reduce((propertyDescriptorMap, key) => {
          propertyDescriptorMap[key] = Object.getOwnPropertyDescriptor(
            objectConfig,
            key
          );
    ...
    try {
      ...
      try {
        if (precontent) {
          ...
          await (
            gnc.is.generatorFunc(precontent) ? gnc.of(precontent) : precontent
          ).call(object, config);
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          if (!infoFile) {
            throw new Error(
              "未找到info.json,导入模块化扩展必须加入info.json！"
            );
          }
      ...
      Object.keys(config).forEach((value) => {
        ...
        if (configObject && "init" in configObject) {
          game.saveConfig(
            `extension_${extensionName}_${value}`,
            configObject.init
          );
        }
      });
      if (typeof game.readFile == "function") {
        const files = zip.files,
          ...
          fileList = Object.keys(files)
            .filter(
              (key) => !files[key].dir && !hiddenFileFlags.includes(key[0])
            )
            .reverse();
        //电脑端
        //具备nodeJS环境
        if (lib.node && lib.node.fs) {
          const writeFile = (errnoException) => {
            ...
            if (fileList.length) {
              ...
              return fileName.length
                ? game.promises
                    .createDir(
                      `extension/${extensionName}/${fileName.join("/")}`
                    )
          ...
          game.promises
            .ensureDirectory(`extension/${extensionName}`)
            .then(writeFile)
            .catch(UHP);
        ...
        localStorage.setItem(
          `${lib.configprefix}extension_${extensionName}`,
          str
        );
        const hiddenFileFlags = [".", "_"],
          fileList = Object.keys(zip.files).filter(
            (filePath) =>
              !hiddenFileFlags.includes(filePath[0]) &&
        ...
        if (players.bool) {
          ...
          for (var i = 0; i < game.players.length; i++) {
            game.players[i].dataset.position =
              parseInt(game.players[i].dataset.position) + 1;
          }
  ...
  prompt() {
    ...
    //}
    //catch(e){
    let promptContainer = ui.create.div(
      ".popup-container",
      ui.window,
      function () {
        if (this.clicked) {
  ...
  zsPlayLineAnimation(name, node, fake, points) {
    ...
    var finish = function () {
      ...
      if (points != undefined) {
        ...
        div2.style.width =
          Math.pow(Math.pow(x1 - x0, 2) + Math.pow(y1 - y0, 2), 0.5) + 2 + "px";
        div2.style.left = x0 + "px";
        div2.style.top = y0 - 10 + "px";
        div2.style.transform =
          "rotate(" + getAngle(x0, y0, x1, y1) + "deg) scaleX(0)";
        ...
        setTimeout(function () {
          ...
          div.style.transform +=
            ...
            "px)";
          div2.style.transform =
            "rotate(" + getAngle(x0, y0, x1, y1) + "deg) scaleX(1)";
        }, 50);
        setTimeout(
          function () {
            ...
            div2.style.transform =
              ...
              (Math.pow(Math.pow(x1 - x0, 2) + Math.pow(y1 - y0, 2), 0.5) +
                2 -
                Math.pow(
                  Math.pow(div.offsetHeight / 2, 2) +
                    Math.pow(div.offsetWidth / 2, 2),
                  0.5
                )) +
              "px) scaleX(0.01)";
          },
      ...
      for (let j in pack[i]) {
        if (i == "character") {
          ...
          if (
            ...
            lib.config.forbidai_user.includes(j)
          ) {
            lib.config.forbidai.add(j);
          }
  ...
  createCard(name, suit, number, nature) {
    ...
    if (!suit && lib.card && lib.card[name] && lib.card[name].cardcolor) {
      suit = lib.card[name].cardcolor;
    }
    if (!nature && lib.card && lib.card[name] && lib.card[name].cardnature) {
  ...
  over(result, bool) {
    ...
    for (let i = 0; i < game.players.length; i++) {
      if (
        !_status.connectMode &&
        game.players[i].isUnderControl(true) &&
        game.layout != "long2"
      ) {
        continue;
      }
    ...
    for (let i = 0; i < game.dead.length; i++) {
      if (
        !_status.connectMode &&
        game.dead[i].isUnderControl(true) &&
        game.layout != "long2"
      ) {
        continue;
      }
      ...
      if (hs.length) {
        dialog.add(
          '<div class="text center">' + get.translation(game.dead[i]) + "</div>"
        );
        dialog.addSmall(hs);
      }
    }
  ...
  check(event = _status.event) {
    ...
    let useCache =
      !lib.config.compatiblemode && !event.skill && !event.multitarget;
    ...
    if (
      ...
      if (
        event.deadTarget ||
        (event.skill && get.info(event.skill)?.deadTarget) ||
        (card && get.info(card)?.deadTarget)
      ) {
    ...
    game.loadModeAsync(name, async (exports) => {
      ...
      if (_status.connectMode && lib.mode[name].connect) {
        ...
        if (configx) {
          ...
          lib.configOL.bannedcards =
            lib.config["connect_" + name + "_bannedcards"];
        }
  ...
  finishSkill(i, sub) {
    ...
    if (
      _status.mode &&
      lib.translate[iInfo + "_" + mode + "_" + _status.mode]
    ) {
      lib.translate[iInfo] =
        lib.translate[iInfo + "_" + mode + "_" + _status.mode];
    ...
    if (info.round) {
      ...
      lib.translate[`${k}_bg`] =
        lib.translate[`${i}_bg`] || lib.translate[k][0];
    }
    if (info.marktext) {
  ...
  addFellow(position, character, animation) {
    ...
    const player = ui.create
      .player(ui.arena)
      .addTempClass(animation || "start");
  ...
  filterSkills(skills, player, exclude) {
    ...
    return out.filter(
      (value) =>
        (exclude && exclude.includes(value)) || !get.is.blocked(value, player)
    );
  }
  /**
   * @param { string[] } skills
      ...
      skills.reduce((previousValue, currentValue) => {
        ...
        if (info) {
          ...
          if (subSkill && info.subSkill) {
            const adds = Object.keys(info.subSkill)?.filter(
              (i) => lib.skill[`${currentValue}_${i}`]
            );
            if (adds?.length) {
  ...
  filterPlayer2(func, list, includeOut) {
    ...
    return list.addArray(
      game.players
        .concat(game.dead)
        .filter((value) => (includeOut || !value.isOut()) && func(value))
    );
  }
  /**
   * @param { (player: Player) => boolean } func
  ...
  findPlayer2(func, includeOut) {
    return (
      game.players
        .concat(game.dead)
        .find((value) => (includeOut || !value.isOut()) && func(value)) || null
    );
  }
  /**
   * @param { (player: Player) => boolean } func
