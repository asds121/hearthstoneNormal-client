        ...
        if (nameinfo.img) {
          ...
          for (const value of nameinfo.trashBin) {
            if (value.startsWith("img:")) {
              ...
              throw new Error(
                `Invalid URL scheme: ext: is not allowed. Found: ${value}`
              );
...
HTMLDivElement.prototype.setBackgroundImage = function (img) {
  // Helper function to resolve resource path based on extension assets.json
  const resolveResourcePath = (resourcePath) => {
    ...
    if (extensionName) {
      try {
        ...
        console.warn(
          `[setBackgroundImage] Failed to resolve extension asset path:`,
          e
        );
      }
    }
  ...
  if (Array.isArray(img)) {
    this.style.backgroundImage = img
      .unique()
      .map((v) => {
        if (typeof v === "string" && v.startsWith("ext:")) {
          throw new Error(
            `Invalid URL scheme: ext: is not allowed. Found: ${v}`
          );
        }
        const resolvedPath = resolveResourcePath(v);
        return `url("${lib.assetURL}${resolvedPath}")`;
      })
      .join(",");
  } else if (URL.canParse(img)) {
    this.style.backgroundImage = `url("${img}")`;
  } else if (typeof img === "string" && img.startsWith("ext:")) {
    throw new Error(`Invalid URL scheme: ext: is not allowed. Found: ${img}`);
  } else {
    const resolvedPath = resolveResourcePath(img);
    this.style.backgroundImage = `url("${lib.assetURL}${resolvedPath}")`;
  }
  return this;
};
/**
