 ...
 */
export const importCardPack = generateImportFunction("card", (name) => {
  const alreadyModernCardPack = lib.config.moderned_cards || [];

  return alreadyModernCardPack.includes(name)
    ? `../../card/${name}/index.js`
    : `../../card/${name}.js`;
});
...
export const importMode = generateImportFunction("mode", (name) => {
  ...
  return alreadyModernMode.includes(name)
    ? `../../mode/${name}/index.js`
    : `../../mode/${name}.js`;
});
...
function generateImportFunction(type, pathParser) {
  return async (name) => {
    ...
    if (modeContent.type !== type) {
      throw new Error(
        `Loaded Content doesn't conform to "${type}" but "${modeContent.type}".`
      );
    }
    // @ts-expect-error ignore
...
async function createEmptyExtension(name) {
  const extensionInfo = await lib.init.promises
    .json(`${lib.assetURL}extension/${name}/info.json`)
    .then(
      (info) => info,
      () => {
        return {
  ...
  return {
    ...
    package: {
      ...
      intro: extensionInfo.intro
        ? extensionInfo.intro.replace("${assetURL}", lib.assetURL)
        : "",
