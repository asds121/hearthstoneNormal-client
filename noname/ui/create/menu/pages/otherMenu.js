...
export const otherMenu = function (
  /** @type { boolean | undefined } */ connectMenu
) {
  if (connectMenu) {
    return;
  }
  ...
  var playButton = ui.create.div(
    ".menubutton.round.highlight.hidden",
    "播",
    start
  );
  ...
  var deleteButton = ui.create.div(
    ".menubutton.round.highlight.hidden",
    "删",
    start
  );
  ...
  (function () {
    ...
    (function () {
      ...
      var buttonx = ui.create.node("button", "访问项目主页", function () {
        window.open("https://github.com/asds121/hearthstoneNormal-client");
      });
  ...
  (function () {
    ...
    node._initLink = function () {
      ...
      if (lib.device === "android") {
        ...
        if (
          typeof window.NonameAndroidBridge?.getPackageVersionCode ===
          "function"
        ) {
      ...
      for (var i = 0; i < lib.config.all.mode.length; i++) {
        ...
        if (lib.config.gameRecord[lib.config.all.mode[i]].str) {
          ...
          var item = ui.create.div(
            ".config.indent",
            lib.config.gameRecord[lib.config.all.mode[i]].str +
              "<span>重置</span>",
            page
          );
          ...
          importVideo.lastChild.onclick = function () {
            ...
            fileReader.onload = function (fileLoadedEvent) {
              ...
              for (var deletei = 0; deletei < 5; deletei++) {
                if (
                  videos.length >= parseInt(lib.config.video) &&
                  videos.length
                ) {
  ...
  for (const [name, content] of Object.entries(lib.help)) {
    ...
    if (typeof content == "object") {
      ...
      // 如果对象拥有"data"方式或"setup"方式，则视为vue组件
        ...
        typeof contentObject.data == "function" ||
        typeof contentObject.setup == "function"
      ) {
        // 创建vue组件
