...
export const cardPackMenu = function (connectMenu) {
  ...
  var recreatePile = function () {
    lib.config.customcardpile["当前牌堆"] = [
  ...
  var createModeConfig = function (mode, position) {
    ...
    node._initLink = function () {
      ...
      if (
        !connectMenu &&
        !lib.config.all.sgscards.includes(mode) &&
        !mode.startsWith("mode_")
      ) {
        ui.create.div(
          ".config.pointerspan",
          ...
          function () {
            if (this.firstChild.innerHTML == "隐藏卡牌包") {
              ...
              if (!lib.config.prompt_hidepack) {
                console.warn("隐藏的扩展包可通过选项-其它-重置隐藏内容恢复");
      ...
      if (
        (!mode.startsWith("mode_") || (cardPack && cardPack.closeable)) &&
        lib.cardPile[mode]
      ) {
        var cardpileNodes = [];
        ...
        var cfgnode = ui.create.div(
          page,
          ".config.pointerspan.cardpilecfg.toggle"
        );
        var cfgaddcard = ui.create.node(
          "button",
          "",
          "添加卡牌",
          cfgnode,
        ...
        var cfgenablecard = ui.create.node(
          "button",
          "",
          "全部开启",
          cfgnode,
        ...
        var cardpileadd = ui.create.div(
          ".config.toggle.hidden.cardpilecfg.cardpilecfgadd",
          page
        );
        var pileaddlist = [];
        ...
        var cardpileaddname = ui.create.selectlist(
          pileaddlist,
          null,
          cardpileadd
        );
  ...
  (function () {
    ...
    start.firstChild.insertBefore(
      node,
      start.firstChild.querySelector(".lefttext")
    );
    node.link = page;
    node.mode = "cardpile";
    node.create = function () {
      ...
      var pileChoose = ui.create.div(
        ".config.toggle.cardpilecfg.nomarginleft",
        "选择牌堆",
        page
      );
      createList();
      ...
      ui.create.div(
        "",
        '名称：<input type="text"><button>确定</button>',
        exportCardPile
      );
      ...
      exportCardPile.firstChild.lastChild.onclick = function () {
        ...
        var ok = true;
        if (
          lib.config.customcardpile[name] ||
          name == "默认牌堆" ||
          name == "当前牌堆"
        ) {
  ...
  return function (packName) {
    ...
    // 显示不是无名杀自带的卡牌包
    if (
      !lib.connectCardPack.includes(packName) &&
      !lib.config.all.cards.includes(packName)
    ) {
      if (
        !(connectMenu && ["mode_derivation", "mode_banned"].includes(packName))
      ) {
