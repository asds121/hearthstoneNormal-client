...
export class Create {
    ...
    const createMenu = function (pos, self, List, click) {
      ...
      if (List && List.length && click) {
        for (let i = 0; i < List.length; ++i) {
          ...
          let className =
            "CodeMirror-hint" + (i != 0 ? "" : " " + "CodeMirror-hint-active");
  ...
  characterDialog() {
    ...
    dialog.supportsPagination = Boolean(
      parseInt(lib.config.showMax_character_number)
    );
    ...
    const updatePagination = () => {
      if (dialog.paginationMaxCount.get("character")) {
        ...
        if (p) {
          ...
          // @ts-expect-error ignore
          p.setTotalPageCount(
            Math.ceil(array.length / dialog.paginationMaxCount.get("character"))
          );
        }
      }
    };
    ...
    const updateFind = () => {
      ...
      for (let btn of dialog.buttons) {
        if (
          reg.test(get.translation(btn.link)) ||
          reg.test(get.translation(btn.link + "_ab"))
        ) {
    ...
    if (dialog.paginationMaxCount.get("character")) {
      ...
      const array = dialog.buttons.filter(
        (item) =>
          !item.classList.contains("nodisplay") && item.style.display !== "none"
      );
      // 传入初始配置 + 渲染元素
      ...
      async function () {
        ...
        function statistic(cards, title) {
          ...
          function renderNumberColumn() {
            let numberResult = Object.groupBy(cards, (card) =>
              get.number(card)
            );
            for (let i = 1; i <= 13; i++) {
              if (!numberResult[i]) {
          ...
          function renderTypeColumns() {
            ...
            for (let key of Object.keys(typeResult).sort((a, b) => {
              ...
              let result = Object.groupBy(typeResult[key], (card) =>
                get.name(card)
              );
              if (key == "basic") {
                Object.assign(
    ...
    skill: (item, type, position, noclick, node) => {
      ...
      node._customintro = function (uiintro, evt) {
        ...
        if (lib.translate[skill + "_info"]) {
          uiintro.add(
            `<div class="text">${get.skillInfoTranslation(skill)}</div>`
          );
  ...
  me(hasme) {
    ...
    if (lib.config.mousewheel && !lib.config.touchscreen) {
      ...
      ui.handcards1Container.addEventListener("wheel", ui.click.mousewheel, {
        passive: true,
      });
      ui.handcards2Container.addEventListener("wheel", ui.click.mousewheel, {
        passive: true,
      });
    }
