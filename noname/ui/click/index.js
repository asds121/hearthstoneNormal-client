...
export class Click {
      ...
      uiintro.refresh = function () {
        ...
        timeconfirm.onclick = function () {
          ...
          if (get.is.banWords(button.input.value)) {
            ...
            ui.create.div(
              "",
              "已有" + fakeinfo.members.length + "人加入",
              eventnode
            );
      ...
      uiintro.refresh = function () {
        ...
        for (var i = 0; i < button.info.length; i++) {
          ...
          if (button.info[i][4] == game.wsid) {
            ui.create.div(
              ".name",
              '<span class="thundertext thunderauto">' +
                (button.info[i][0] || "无名玩家"),
              node
            );
            node.isme = true;
  ...
  chat() {
    ...
    var node = uiintro.add(
      '<input type="text" value="' + _status.chatValue + '">'
    );
  ...
  charactercard(name, sourcenode, noedit, resume, avatar, audioName) {
    ...
    const refreshIntro = function () {
      ...
      if (
        ...
        nameinfo.skills.forEach((skill) => {
          let voiceMap = get.Audio.skill({
            skill,
            player: bg.tempSkin || audioName,
          }).textList;
          if (!voiceMap.length) {
          ...
          if (info.derivation) {
            ...
            for (var i = 0; i < derivation.length; i++) {
              ...
              if (!derivationVoiceMap.length) {
                derivationVoiceMap = get.Audio.skill({
                  skill: derivation[i],
                  player: name,
                }).textList;
              }
        ...
        if (
          dieAudios.length ||
          skillAudioMap.size > 0 ||
          derivationSkillAudioMap.size > 0
        ) {
        ...
        clickSkill = function (e) {
          ...
          if (this.link != "dieAudios") {
            ...
            let dieAudios = get.Audio.die({
              player: { name: this.playername, skin: { name: name } },
            })
              .audioList.map((i) => i.text)
              .filter(Boolean);
            intro2.innerHTML =
        ...
        if (showCharacterNamePinyin != "doNotShow") {
          ...
          const characterSexDiv = ui.create.div(
            ...
            exInfoSex =
              exInfo && exInfo.find((value) => value.startsWith("sex:")),
    ...
    if (lib.characterSubstitute[name]) {
            ...
            for (let i of list) {
              ...
              if (!lib.character[i] && skinList.some((skin) => skin[0] == i)) {
                ...
                lib.character[i] = [
                  ...
                  0,
                  [],
                  (skinList.find((skin) => skin[0] == i) || [i, []])[1],
                ];
              }
              button.name = i;
      ...
      skin.list = [
        name,
        ...lib.characterSubstitute[name].map((skin) => skin[0]),
      ];
    }
  ...
  mousewheel(evt) {
    ...
    if (evt.detail > 0 || evt.wheelDelta < 0) {
      node.interval = setInterval(function () {
        if (
          num-- &&
          Math.abs(node.scrollLeft + node.clientWidth - node.scrollWidth) > 0
        ) {
