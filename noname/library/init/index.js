...
export class LibInit {
  ...
  connection(ws) {
    ...
    if (window.isNonameServer) {
      document.querySelector("#server_count").innerHTML =
  ...
  req(str, onload, onerror, master) {
    ...
    if (str.startsWith("http")) {
      ...
      // 删除了对远程服务器的请求，只支持http和local资源
      const e = new Error("只支持http和local资源");
      if (typeof onerror == "function") {
        onerror(e);
      } else {
        throw e;
  ...
  cssstyles() {
    ...
    if (lib.config.cardtext_font && lib.config.cardtext_font != "default") {
      ui.css.styles.sheet.insertRule(
        ".card div:not(.info):not(.background) {font-family: " +
          lib.config.cardtext_font +
          ";}",
        0
      );
    }
    ...
    new Promise((resolve) => setTimeout(resolve, 500))
      .then(() => {
        if (game.layout == "default") {
          ...
          ui.css.layout.href =
            lib.assetURL + "layout/" + game.layout + "/layout.css";
        }
  ...
  getCurrentFileLocation(url) {
    let head = window.location.href.slice(
      0,
