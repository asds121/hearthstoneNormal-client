    ...
    appearence: {
      ...
      config: {
          ...
          onclick: async (theme) => {
            ...
            if (lib.config.autostyle) {
              if (theme === "simple") {
                ...
                lib.configMenu.appearence.config.player_border.onclick(
                  "normal"
                );
              }
            }
        ...
        change_skin: {
          ...
          init: true,
          intro:
            "在武将的右键菜单中换肤，皮肤可在选项-文件-图片文件-皮肤图片中添加",
        },
        change_skin_auto: {
          name: "自动换肤",
          init: "off",
              ...
              function () {
                if (confirm("确定删除自定义图片？（此操作不可撤销）")) {
                  ...
                  if (lib.config.hp_style == "custom") {
                    lib.configMenu.appearence.config.hp_style.onclick(
                      "default"
                    );
                    switcher.lastChild.innerHTML = "默认";
                  }
  ...
  codeMirrorReady(node, editor) {
    ...
    function scriptHint(editor, keywords, getToken, options) {
        ...
        try {
          const code =
            context.length == 1
              ? context[0].string
              : context.reduceRight(
                  (pre, cur) => (pre.string || pre) + "." + cur.string
                );
      ...
      return {
        list: [
          ...new Set(
            getCompletions(token, context, keywords, options).concat(list)
          ),
        ]
          .filter((key) => key.startsWith(token.string))
  ...
  skill = {
    ...
    _stratagem_add_buff: {
      ...
      filter: (event, player) => {
        ...
        if (
          !stratagemSettings ||
          (!stratagemSettings.roundOneUseFury && game.roundNumber < 2)
        ) {
          return false;
        }
      ...
      check: (card) => {
        ...
        return (_status.event.getParent().shanRequired || 1) > 1 &&
          get.damageEffect(
            player,
            _status.event.getParent().player || player,
            player
          ) < 0
          ? 1
          : 0;
        ...
        gameEvent.pushHandler("onNextUseCard", (event, option) => {
          if (event.step == 0 && option.state == "end") {
            game.broadcastAll(
              (cards) =>
                cards.forEach((card) =>
                  card.clone.classList.add("stratagem-fury-glow")
                ),
              event.cards
            );
          }
        ...
        damage: {
          ...
          content: function () {
            ...
            for (var info of storage) {
              if (
                info.type == "damage" &&
                (source == player || source == info.target)
              ) {
                if (!info.damage) {
                  info.damage = 0;
                }
    ...
    zhengsu: {
      ...
      subSkill: {
        leijin: {
          ...
          filter: function (event, player) {
            return (
              player.isPhaseUsing() && player.storage.zhengsu_leijin !== false
            );
          },
          content: function () {
        ...
        bianzhen: {
          ...
          filter: function (event, player) {
            return (
              player.isPhaseUsing() && player.storage.zhengsu_bianzhen !== false
            );
          },
          content: function () {
            var list = player.getHistory("useCard", function (evt) {
    ...
    aozhan: {
      ...
      mod: {
        targetEnabled: function (card) {
          if (
            card.name == "tao" &&
            ((card.isCard && card.cardid) || get.itemtype(card) == "card")
          ) {
            return false;
          }
        },
          ...
          content: function () {
            ...
            for (var i = 0; i < list.length; i++) {
              if (list[i] && lib.character[list[i]]) {
                ...
                if (
                  info.skills.includes("dualside") &&
                  info.dualSideCharacter
                ) {
        ...
        player.recast(cards, void 0, (player, cards) => {
          ...
          cards.forEach((value) => {
            if (
              ...
              var libCard = get.libCard(
                (info) => info.subtype == "spell_bronze"
              );
              if (!libCard.length) {
                return;
              }
    ...
    dongcha: {
      ...
      filter(event, player) {
        return game.hasPlayer(
          (current) => current.countDiscardableCards(player, "ej") > 0
        );
      },
      charlotte: true,
      forceunique: true,
