...
export const Content = {
  ...
  async changeCharacter(event, trigger, player) {
    ...
    if (event.changeGroup !== false) {
      ...
      if (!player.isUnseen(1)) {
        newGroups = [get.character(player.name1, 1)];
      } else if (player.name2 && !player.isUnseen(2)) {
        newGroups = [get.character(player.name2, 1)];
      }
  ...
  addShownCards: () => {
    const hs = player.getCards("h"),
      showingCards = event._cards.filter((showingCard) =>
  ...
  hideShownCards: () => {
    var shown = player.getShownCards(),
      hidingCards = event._cards.filter((hidingCard) =>
            ...
            .set("filterOk", function () {
              ...
              return (
                ui.selected.buttons.reduce(function (num, button) {
                  ...
                  return (
                    num + get.numOf(get.subtypes(button.link, false), evt.slot)
                  );
                }, 0) == evt.required
              );
            })
  ...
  replaceEquip: async function (event, trigger, player) {
    let vcards = [];
    vcards.push(
      event.card[event.card.cardSymbol]
        ? event.card[event.card.cardSymbol]
        : get.autoViewAs(event.card, void 0, false)
    );
  ...
  replaceEquip_old: function () {
    ...
    if (lose <= 0) {
      ...
      var cards = player.getEquips(slot).filter((card) => {
        return (
          !event.cards.includes(card) && lib.filter.canBeReplaced(card, player)
        );
      });
      if (cards.length > 0) {
        if (lose >= left) {
...
player.removeVirtualEquip(card);
  ...
  equip_old: function () {
    ...
    if (get.itemtype(result) == "cards") {
      player
        .lose(result, "visible")
        .set("type", "equip")
        .set("getlx", false).swapEquip = true;
      if (info.loseThrow) {
  ...
  chooseToDebate: function () {
    ...
    if (!event.targets.length) {
      ...
      if (event.fixedResult) {
        event.targets = event.targets.removeArray(
          event.fixedResult.map((i) => i[0])
        );
      }
      if (event.targets.length) {
        var next = player
          .chooseCardOL(
            event.targets,
            get.translation(player) + "发起了议事，请选择展示的手牌",
            true
          )
          .set("type", "debate")
          .set("source", player)
    ...
    opinion =
      event[opinion[0]].length > event[opinion[1]].length ? opinion[0] : null;
    if (opinion) {
      game.log(
        player,
        "本次发起的议事结果为",
  ...
  chooseCooperationFor: function () {
    ...
    ("step 1");
    if (result.bool) {
      player.cooperationWith(
        target,
        result.links[0][2].slice(12),
        event.reason
      );
    }
  },
  ...
  chooseToPlayBeatmap: function () {
    ...
    game.broadcastAll(
      function (player, id, beatmap) {
        ...
        var str =
          get.translation(player) + "正在演奏《" + beatmap.name + "》...";
        if (!_status.connectMode) {
    ...
    if (event.isMine()) {
      ...
      var addNode = function () {
        ...
        node.style.left =
          Math.ceil((width * node_pos) / number_of_tracks + 5) + "px";
          ...
          Math.sqrt(
            choose_to_play_beatmap_accuracies.reduce(
              (previousValue, currentValue) =>
                previousValue + Math.pow(currentValue - mean, 2),
              0
            )
          ) / 2
        );
      ...
      result = await new Promise((resolve) => {
        ...
        var dragStart = function (e) {
          ...
          if (target) {
            ...
            touchStartY =
              ...
              game.documentZoom;
            elementOffsetX =
              target.getBoundingClientRect().x / game.documentZoom -
              touchStartX;
            elementOffsetY =
              target.getBoundingClientRect().y / game.documentZoom -
              touchStartY;
        ...
        var dragEnd = function (e) {
          ...
          var clientX =
            (e instanceof MouseEvent
              ? e.clientX
              : e.changedTouches[0].clientX) / game.documentZoom;
          var clientY =
            (e instanceof MouseEvent
              ? e.clientY
              : e.changedTouches[0].clientY) / game.documentZoom;
          // 鼠标当前处于哪个元素上
          var target = document.elementFromPoint(
            clientX * game.documentZoom,
            clientY * game.documentZoom
          );
                ...
                buttonss
                  ...
                  .map((element) => {
                    return new Promise((resolve) => {
                      ...
                      if (
                        transformValue !== "translateX(0px) translateY(0px)" &&
                        transformValue !== ""
                      ) {
        ...
        for (var i = 0; i < list.length; i++) {
          var tex = event.dialog.add(
            '<div class="text center">' + list[i][0] + "</div>"
          );
  ...
  chooseUseTarget: function () {
    ...
    if (info.notarget || range[1] <= -1) {
      ...
      next.set(
        "prompt",
        event.prompt || "选择" + get.translation(card) + "的目标"
      );
      if (event.prompt2) {
        next.set("prompt2", event.prompt2);
      }
  ...
  chooseToDuiben: function () {
      ...
      function (list, translationList = []) {
        ...
        for (var i = 0; i < 4; i++) {
          lib.card[list2[i]].image =
            "card/" + list2[i] + (list[0] == "全军出击" ? "" : "_" + list[i]);
    ...
    target.$compare(
      game.createCard(event.tes, "", ""),
      player,
      game.createCard(event.mes, "", "")
    );
    ...
    const skill =
      ...
      "_" +
      (event.result.bool ? "true" + mes : "false");
    ...
    if (event.selectButton) {
      ...
      if (!list.length) {
        ...
        if (selectButton[0] == selectButton[1]) {
          ...
          str +=
            get.cnNumber(selectButton[0]) +
            "至" +
            get.cnNumber(selectButton[1]);
        }
        str += "个装备栏";
        var next = source.chooseButton(selectButton, true, [
          str,
          [list, "tdnodes"],
        ]);
  ...
  loadPackage: function () {
    ...
    for (i in character) {
      ...
      if (
        character[i].forbid &&
        character[i].forbid.includes(lib.config.mode)
      ) {
        continue;
      }
      if (
        character[i].mode &&
        character[i].mode.includes(lib.config.mode) == false
      ) {
        continue;
      }
        ...
        for (k in character[i][j]) {
          if (j == "character") {
            ...
            if (
              ...
              lib.config.forbidai_user.includes(k)
            ) {
              lib.config.forbidai.add(k);
            }
          ...
          if (j == "translate" && k == i) {
            ...
            if (lib[j][k] == undefined) {
              ...
              Array.isArray(lib[j][k]) &&
              Array.isArray(character[i][j][k])
            ) {
        ...
        if (usableSkills.length == 0) {
          ...
          if (get.itemtype(event.doing.player) !== "player") {
            ...
            const silentSkill = event.choice.find((item) => {
              ...
              return (
                skillInfo && (skillInfo.silent || !lib.translate[item.skill])
              );
            });
            if (silentSkill) {
          ...
          if (
            ...
            for (let i = 0; i < event.doing.todoList.length; i++) {
              if (event.current.skill === event.doing.todoList[i].skill) {
                event.doing.doneList.push(
                  event.doing.todoList.splice(i--, 1)[0]
                );
              }
            }
          }
  ...
  createTrigger: function () {
    ...
    if (hidden.includes(event.skill)) {
      ...
      Object.keys(player.additionalSkills).every((i) => {
        ...
        return !game
          .expandSkills(player.additionalSkills[i])
          .includes(event.skill);
      })
    ) {
      event.finish();
    ...
    if (
      player._hookTrigger.some((i) => {
        ...
        return (
          info && info.after && info.after(event, player, event.triggername)
        );
      })
    ) {
  ...
  playVideoContent: function () {
    ...
    if (event.video.length) {
      ...
      if (content.type == "delay") {
        ...
        game.videoContent[content.type](
          game.playerMap[content.player],
          content.content
        );
      } else {
  ...
  chooseToUse: function () {
    ...
    if (event.type == "phase") {
      if (event.isMine()) {
        event.endButton = ui.create.control(
          "结束回合",
          "stayleft",
          function () {
            var evt = _status.event;
    ...
    if (event.player.isUnderControl() && !_status.auto) {
      ...
      var ok = game.check();
      const eventinfo =
        get.info(get.card() || {}) || get.info(event.skill) || {};
      ...
      if (event.result.skill) {
        ...
        if (info && info.chooseButton) {
          ...
          if (info.chooseButton.chooseControl) {
            ...
            next.set(
              "complexSelect",
              info.chooseButton.complexSelect !== false
            );
    ...
    if (
      (!event.result?.bool || event.result?._noHidingTimer) &&
    ...
    if (directFilter && (range[0] >= cards.length || range[1] <= -1)) {
      ...
      if (Array.isArray(event.dialog)) {
        ...
        if (typeof event.prompt == "string") {
          ...
          const position =
            event.position == "h" ? "手" : event.position == "e" ? "装备" : "";
  ...
  chooseToDiscard: [
    async (event, trigger, player) => {
      ...
      if (
        !player
          .getCards(event.position)
          .filter((card) => event.filterCard(card, player)).length
      ) {
        event.result = {
          bool: false,
          cards: [],
    ...
    async (event, trigger, player) => {
      if (event.result == "ai") {
        game.check();
        if (
          (ai.basic.chooseCard(event.ai) || event.forced) &&
          (!event.filterOk || event.filterOk())
        ) {
          ui.click.ok();
        } else if (event.skill) {
  ...
  discardMultiple: function () {
    ...
    for (var i = 0; i < event.lose_list.length; i++) {
      var next = event.lose_list[i][0].lose(
        event.lose_list[i][1],
        event.position
      );
  ...
  chooseToCompareMeanwhile: function () {
    ...
    for (var i = 0; i < targets.length; i++) {
      if (
        targets[i].countCards("h") == 0 &&
        (!event.fixedResult || !event.fixedResult[targets[i].playerid])
      ) {
        event.result = { cancelled: true, bool: false };
        event.finish();
        return;
    ...
    if (event.fixedResult && event.fixedResult[player.playerid]) {
      ...
      if (
        result[0].skill &&
        lib.skill[result[0].skill] &&
        lib.skill[result[0].skill].onCompare
      ) {
  ...
  chooseToCompareMultiple: function () {
    ...
    var str;
    if (
      event.forceWinner === player ||
      (event.forceWinner !== target && event.num1 > event.num2)
    ) {
      event.winner = player;
  ...
  chooseToCompare: [
    ...
    async (event, trigger, player, result) => {
      ...
      if (event.list.includes(target)) {
        ...
        if (
          result[index].skill &&
          lib.skill[result[index].skill] &&
          lib.skill[result[index].skill].onCompare
        ) {
          target.logSkill(result[index].skill);
          result[index].cards =
            lib.skill[result[index].skill].onCompare(target);
        }
    ...
    async (event, trigger, player) => {
      //const {target} = event;
      if (
        typeof event.target.ai.shown == "number" &&
        event.target.ai.shown <= 0.85 &&
        event.addToAI
      ) {
  ...
  chooseButton: function () {
    ...
    if (event.isMine()) {
      if (
        ...
        (event.direct && num == selectButton[0]) ||
        event.forceDirect
      ) {
  ...
  chooseTarget: function () {
    ...
    if (event.result == "ai") {
      game.check();
      if (
        (ai.basic.chooseTarget(event.ai) || forced) &&
        (!event.filterOk || event.filterOk())
      ) {
        ui.click.ok();
      } else {
    ...
    async (event, _trigger, player, result) => {
      ...
      if (event.result == "ai") {
        ...
        if (ai.basic.chooseButton(event.ai1) || event.forced) {
          if (
            (ai.basic.chooseTarget(event.ai2) || event.forced) &&
            (!event.filterOk || event.filterOk())
          ) {
            ui.click.ok();
            _status.event._aiexclude.length = 0;
  ...
  useCard: function () {
    ...
    if (event.animate != false && event.line != false && !event.hideTargets) {
      if (card.name == "wuxie" && event.getParent()._info_map) {
        ...
        var lining =
          (evtmap.multitarget ? evtmap.targets : evtmap.target) || event.player;
    ...
    if (targets.length && !event.hideTargets) {
      var str =
        targets.length == 1 && targets[0] == player ? "#b自己" : targets;
    ...
    if (
      targets[num] &&
      info.ignoreTarget &&
      info.ignoreTarget(card, player, targets[num])
    ) {
  ...
  useSkill: function () {
    ...
    if (targets && targets.length && info.log != "notarget") {
      str +=
        ...
        (targets[0] == player ? "自己" : get.translation(targets[0]));
      for (var i = 1; i < targets.length; i++) {
        str +=
          "、" + (targets[i] == player ? "自己" : get.translation(targets[i]));
      }
      str += "</span>";
    }
    str += "发动了";
    ...
    if (document.getElementsByClassName("thrown").length) {
      if (
        event.skill &&
        get.info(event.skill).delay !== false &&
        get.info(event.skill).delay !== 0
      ) {
        game.delayx();
      }
    } else {
    ...
    async (event, trigger, player) => {
      ...
      if (event.animate == "draw") {
        ...
        if (event.animate == "give") {
          for (var i in evtmap) {
            var source = (_status.connectMode ? lib.playerOL : game.playerMap)[
              i
            ];
      ...
      for (var i = 0; i < cards.length; i++) {
        ...
        for (var j = 0; j < cardx.length; j++) {
          if (cardx[j].gaintag && cardx[j].gaintag.length) {
            ...
            //仅移除非永久标记
            const tags = cardx[j].gaintag.filter(
          ...
          if ("_destroy" in cardx[j]) {
            ...
            if (
              event.getlx !== false &&
              event.position &&
              cardx[j].willBeDestroyed(event.position.id, null, event)
            ) {
          ...
          if (event.position) {
            ...
            if (event.insert_index) {
              ...
              event.position.insertBefore(
                cardx[j],
                event.insert_index(event, cardx[j])
              );
  ...
  damage: function () {
    ...
    if (player.hujia > 0 && !player.hasSkillTag("nohujia") && !event.nohujia) {
      ...
      if (!damageAudioInfo || damageAudioInfo == "normal") {
        ...
        damageAudioInfo =
          "effect/hujia_damage_" + event.nature + (num > 1 ? "2" : "") + ".mp3";
      } else {
      ...
      if (!damageAudioInfo || damageAudioInfo == "normal") {
        ...
        damageAudioInfo =
          "effect/damage_" + event.nature + (num > 1 ? "2" : "") + ".mp3";
      } else {
        damageAudioInfo = damageAudioInfo[num > 1 ? 2 : 1];
      }
      game.broadcastAll(function (damageAudioInfo) {
    ...
    if (event.animate !== false) {
      ...
      var numx = player.hasSkillTag("nohujia")
        ? num
  ...
  dying: function () {
    ...
    if (player.hp > 0 || event.nodying) {
      ...
      for (var i = 0; i < starts.length; i++) {
        if (
          get.itemtype(starts[i]) == "player" &&
          game.players.concat(game.dead).includes(starts[i])
        ) {
          start = starts[i];
          break;
        }
  ...
  addJudge_old: function () {
    ...
    if (
      !lib.card[viewAs] ||
      (!lib.card[viewAs].effect && !lib.card[viewAs].noEffect)
    ) {
      ...
      game.broadcast(
        function (player, card, viewAs) {
          ...
          if (
            card.clone &&
            (card.clone.parentNode == player.parentNode ||
      ...
      if (cards[0].viewAs && cards[0].viewAs != cards[0].name) {
        if (
          cards[0].classList.contains("fullskin") ||
          cards[0].classList.contains("fullborder")
        ) {
          cards[0].classList.add("fakejudge");
          cards[0].node.background.innerHTML =
            lib.translate[cards[0].viewAs + "_bg"] ||
        ...
        if (lib.card[viewAs].blankCard) {
          game.log(
            player,
            '被扣置了<span class="yellowtext">' +
              get.translation(cards[0].viewAs) +
              "</span>"
          );
        } else {
          game.log(
      ...
      game.addVideo("addJudge", player, [
        get.cardInfo(cards[0]),
        cards[0].viewAs,
      ]);
    }
    if (event.updatePile) {
    ...
    if (event.isMine()) {
      ...
      function clickItem(card, container, allContainers) {
        ...
        if (event.dialog.selectedCard) {
          if (
            card !== event.dialog.selectedCard &&
      ...
      function clickItemContainer(itemContainer, i, allContainers) {
        ...
        let index =
          Array.from(event.dialog.itemContainers).indexOf(itemContainer) / 2 -
          1;
        if (event.filterMove(event.dialog.selectedCard, index, event.moved)) {
          event.dialog.isBusy = true;
          game
            .$elementGoto(event.dialog.selectedCard, itemContainer, 300)
            .then(() => {
              event.dialog.isBusy = false;
              updateButtons();
            });
