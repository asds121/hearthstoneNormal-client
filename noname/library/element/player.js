...
export class Player extends HTMLDivElement {
  ...
  buildNode() {
    ...
    player.node.handcards1._childNodesWatcher = new ChildNodesWatcher(
      player.node.handcards1
    );
    player.node.handcards2._childNodesWatcher = new ChildNodesWatcher(
      player.node.handcards2
    );
      ...
      let observer = new MutationObserver((mutationsList) => {
        for (let mutation of mutationsList) {
          if (mutation.type === "childList") {
            ...
            if (
              addedNodes.some(
                (card) => !card.classList.contains("emptyequip")
              ) ||
              // @ts-expect-error ignore
              removedNodes.some(
                (card) => !card.classList.contains("emptyequip")
  ...
  buildEventListener(noclick) {
    ...
    if (noclick) {
      ...
      node.identity.addEventListener(
        lib.config.touchscreen ? "touchend" : "click",
        ui.click.identity
      );
  ...
  canRespond(event, card, type) {
    ...
    return (
      keys.some((key) =>
        typeof key == "function"
          ? cards.some((card) => key(card, player))
          : player.hasUsableCard(name, type)
  ...
  when(...triggerNames) {
    ...
    if (triggerNames.length == 1) {
      ...
      if (
        Array.isArray(triggerNames[0]) ||
        typeof triggerNames[0] == "string"
      ) {
        ...
        if (typeof translation == "string") {
          ...
          game.broadcastAll(
            (skillName, translation) =>
              (lib.translate[skillName] = translation),
            skillName,
            translation
          );
        }
  ...
  getKnownCards(other = _status.event.player, filter = (card) => true) {
    if (!other) {
      if (other === null) {
        console.trace(
          `getKnownCards的other参数不应传入null,可以用void 0或undefined占位`
        );
      }
    ...
    switch (typeof subtype) {
      case "string":
        if (subtype == "equip3_4") {
          ...
          subtype.startsWith("equip") &&
          parseInt(subtype.slice(5)) > 0
        ) {
          break;
  ...
  disableEquip() {
    ...
    for (var i = 0; i < arguments.length; i++) {
      if (get.itemtype(arguments[i]) == "player") {
        ...
        if (
          arguments[i].startsWith("equip") &&
          parseInt(arguments[i].slice(5)) > 0
        ) {
  ...
  canEquip(name, replace) {
    ...
    for (let range of rangex) {
      ...
      if (!replace) {
        num -= this.getVEquips(range).filter((card) =>
          lib.filter.canBeReplaced(card, this)
        ).length;
      }
      if (num < num2) {
        return false;
  ...
  $changeZhuanhuanji(skill) {
    ...
    if (mark) {
      ...
      // @ts-expect-error ignore
      mark.firstChild.style.transform =
        "rotate(" + parseFloat(mark.firstChild.reversed) + "deg)";
    }
  }
  /**
   * 设置玩家的座位号
  ...
  $throwEmotion(target, name, rotate) {
    ...
    emotion.style.transform =
      "translateY(" + (top2 - top) + "px) translateX(" + (left2 - left) + "px)";
    // @ts-expect-error ignore
    if (["egg", "flower", "shoe"].includes(name) || rotate) {
  ...
  hasUsableCard(name, type) {
    ...
    for (let i = 0; i < skills.length; i++) {
      ...
      if (
        info.viewAs &&
        get.is.object(info.viewAs) &&
        info.viewAs?.name === name
      ) {
          ...
          (typeof info.filter === "function" &&
            evtNames.some((evtName) => {
              ...
              if (
                evt.name === evtName &&
                info[
                  "on" + evtName.slice(0, 1).toUpperCase() + evtName.slice(1)
                ]
              ) {
                info[
                  "on" + evtName.slice(0, 1).toUpperCase() + evtName.slice(1)
              ...
              return evt.name === evtName
                ? info.filter(evt, player, evt.triggername)
                : true;
            }));
        if (goon && bool) {
          return true;
  ...
  isPhaseUsing(notmeisok) {
    ...
    // @ts-expect-error ignore
    return (
      _status.event.name == "phaseUse" ||
      _status.event.getParent("phaseUse").name == "phaseUse"
    );
  }
  /**
   * 与target交换装备区里的牌
  ...
  canCompare(target, goon, bool) {
    ...
    if (
      (!this.countCards("h") && goon !== true) ||
      (!target.countCards("h") && bool !== true)
    ) {
      return false;
    }
    if (
      this.hasSkillTag("noCompareSource") ||
  ...
  init(character, character2, skill, update) {
    ...
    if (
      ...
      this.node.name.querySelectorAll("br").length >= 4
    ) {
  ...
  changeSkin(map, character) {
    ...
    if (!map || !character) {
      console.warn(
        "error: no sourceMap or character to changeSkin",
      ...
      if (this[i] && list) {
        ...
        if (
          (() => {
            ...
            if (
              typeof map.characterName == "string" &&
              this[i] == map.characterName
            ) {
              return true;
            }
            if (
  ...
  initRoom(info, info2) {
    ...
    if (!info || info == "server") {
      ...
      if (info.length) {
        ...
        if (
          config.mode != "guozhan" &&
          (config.mode != "doudizhu" || config.doudizhu_mode != "online")
        ) {
          str += "【";
          for (var i = 0; i < config.cardPack.length; i++) {
            str += get
              .translation(config.cardPack[i] + "_card_config")
              .slice(0, 2);
  ...
  reinit2(newPairs) {
    ...
    if (rawPairs.length == newPairs.length) {
      for (let i = 0; i < Math.min(2, rawPairs.length); i++) {
        ...
        if (
          rawName != newName &&
          lib.character[rawName] &&
          lib.character[newName]
        ) {
  ...
  $reinit(from, to, maxHp, online) {
    ...
    if (this.name2 == to) {
      ...
      if (this == game.me && ui.fakeme) {
        ui.fakeme.style.backgroundImage =
          this.node.avatar.style.backgroundImage;
      }
    }
  }
  uninit() {
  ...
  changeSeat(position, video) {
    ...
    if (
      ...
      player.style.transform =
        "rotate(-90deg) translate(" + -dy + "px," + dx + "px)";
    } else {
  ...
  say(str) {
    ...
    dialog.add(
      '<div class="text" style="word-break:break-all;display:inline">' +
        str +
        "</div>"
    );
      ...
      lib.placePoppedDialog(dialog, {
        clientX:
          (ui.arena.offsetLeft +
            this.getLeft() +
            refnode.offsetLeft +
            refnode.offsetWidth / 2) *
          game.documentZoom,
        clientY:
          (ui.arena.offsetTop +
            this.getTop() +
            refnode.offsetTop +
            refnode.offsetHeight / 4) *
          game.documentZoom,
      });
    } else {
      lib.placePoppedDialog(dialog, {
  ...
  getState() {
    var state = {
      ...
      identityNode: [
        this.node.identity.innerHTML,
        this.node.identity.dataset.color,
      ],
  ...
  flashAvatar(skill, name) {
    ...
    if (
      ...
      lib.character[this.name2].skills.includes(skill)
    ) {
  ...
  $update() {
    ...
    if (!this.storage.nohp) {
      const hidden =
        this.classList.contains("unseen_show") ||
        this.classList.contains("unseen2_show");
      const maxHp = hidden ? 1 : this.maxHp;
      if (maxHp == Infinity) {
        hp.innerHTML =
          this.hp == Infinity
            ? "∞"
            : this.hp + "<br>/<br>" + "∞" + "<div></div>";
        /*
  ...
  removeMark(i, num, log) {
    ...
    this.syncStorage(i);
    this[
      this.storage[i] || (lib.skill[i] && lib.skill[i].mark)
        ? "markSkill"
        : "unmarkSkill"
    ](i);
  ...
  updateMark(i, storage) {
    ...
    if (
      i == "ghujia" ||
      ((!this.marks[i].querySelector(".image") ||
        this.storage[i + "_markcount"]) &&
        lib.skill[i] &&
        lib.skill[i].intro &&
        !lib.skill[i].intro.nocount &&
        (this.storage[i] ||
          this.storage[i + "_markcount"] ||
          lib.skill[i].intro.markcount))
    ) {
      ...
      if (num) {
        ...
        if (!this.marks[i].markcount) {
          this.marks[i].markcount = ui.create.div(
            ".markcount.menubutton",
            this.marks[i]
          );
        }
    ...
    if (type === true) {
      ...
      for (var i = 0; i < history.length; i++) {
        if (!card) {
          ...
          typeof card == "string" &&
          history[i].card &&
          card == history[i].card.name
        ) {
          num++;
        } else if (
          typeof card == "object" &&
          history[i].card &&
          card.name == history[i].card.name
        ) {
          num++;
        }
      }
  ...
  countSkill(skill) {
    ...
    if (
      ...
      this.hasSkill("counttrigger") &&
      this.storage.counttrigger
    ) {
  ...
  getHiddenSkills(unowned, unique) {
    const player = this;
    return player
      .getStockSkills(unowned, unique, true)
      .removeArray(player.getStockSkills(unowned, unique));
  }
  /**
   * @param { string } [arg1='h']
  ...
  *iterableGetCards(arg1, arg2) {
    ...
    for (let i = 0; i < arg1.length; i++) {
      if (arg1[i] == "h") {
        for (let card of get.iterableChildNodes(
          this.node.handcards1,
          this.node.handcards2
        )) {
          if (
  ...
  *iterableGetGainableCards(player, arg1, arg2) {
  ...
  getSkills(arg2, arg3, arg4) {
    ...
    for (let i in this.additionalSkills) {
      if (
        Array.isArray(this.additionalSkills[i]) &&
        (arg2 || i.indexOf("hidden:") !== 0)
      ) {
  ...
  syncStorage(skill) {
    switch (get.itemtype(this.storage[skill])) {
      case "cards":
        game.addVideo("storage", this, [
          skill,
          get.cardsInfo(this.storage[skill]),
          "cards",
        ]);
        break;
      case "card":
        game.addVideo("storage", this, [
          skill,
          get.cardInfo(this.storage[skill]),
          "card",
        ]);
        break;
      default:
        try {
          game.addVideo("storage", this, [
            skill,
            JSON.parse(JSON.stringify(this.storage[skill])),
          ]);
        } catch (e) {
    ...
    if (arguments.length == 1 && get.objtype(arguments[0]) == "object") {
      ...
      for (var i = 0; i < arguments.length; i++) {
        if (
          ...
          get.itemtype(arguments[i]) == "select"
        ) {
  ...
  chooseCardButton() {
    ...
    return this.chooseButton(forced, select, "hidden", [
      prompt,
      cards,
      "hidden",
    ]);
  ...
  chooseVCardButton() {
    ...
    for (var i = 0; i < list.length; i++) {
      list[i] = [
        notype ? "" : get.subtype(list[i], false) || get.type(list[i]),
        "",
        list[i],
      ];
    }
  ...
  chooseCard(choose) {
    ...
    next.autochoose = function () {
      ...
      return (
        get.select(this.selectCard)[0] >= this.player.countCards(this.position)
      );
    };
  ...
  chooseControlList() {
    ...
    return this.chooseControl(forced, func)
      .set("choiceList", list)
      .set("prompt", prompt);
  }
  chooseControl() {
  ...
  chooseNumbers() {
    ...
    if (!next.filterOk) {
      if (next.optionSum) {
        next.filterOk = (event) =>
          event.numbers.reduce((sum, num) => sum + num, 0) <= event.optionSum;
      } else {
        next.filterOk = () => true;
      }
    }
  ...
  useCard() {
      ...
      for (const skill of skills) {
        for (const key of keys) {
          ...
          if (mod) {
            ...
            if (
              result !== undefined &&
              typeof arg[arg.length - 1] !== "object"
            ) {
    ...
    for (var i = 0; i < next.targets.length; i++) {
      if (
        get.attitude(this, next.targets[i]) >= -1 &&
        get.attitude(this, next.targets[i]) < 0
      ) {
  ...
  useSkill() {
    ...
    if (
      next.skill &&
      get.info(next.skill) &&
      get.info(next.skill).changeTarget
    ) {
  ...
  $addToExpansion(cards, broadcast, gaintag) {
    ...
    for (var i = 0; i < cards.length; i++) {
      ...
      this.node.expansions.insertBefore(
        cards[i],
        this.node.expansions.firstChild
      );
    }
    if (broadcast !== false) {
      game.broadcast(
  ...
  directgain(cards, broadcast, gaintag) {
    ...
    for (var i = 0; i < cards.length; i++) {
      ...
      if (get.is.singleHandcard() || sort > 0) {
        this.node.handcards1.insertBefore(
          cards[i],
          this.node.handcards1.firstChild
        );
      } else {
        this.node.handcards2.insertBefore(
          cards[i],
          this.node.handcards2.firstChild
        );
      }
    }
  ...
  equip(card, draw) {
    ...
    if (
      (next.cards.length > 1 &&
        next.cards.some((cardx) => cardx.isViewAsCard)) ||
      !next.card.name
    ) {
  ...
  wait(callback) {
    if (lib.node) {
      ...
      if (this.ws && !this.ws.closed) {
        ...
        if (
          _status.event._global_timer ||
          _status.event.getParent().skillHidden
        ) {
  ...
  unwait(result) {
    ...
    for (var i in lib.node.torespond) {
      if (lib.node.torespond[i] == "_noname_waiting") {
        ...
        lib.node.torespond[i] &&
        lib.node.torespond[i]._noname_waiting
      ) {
        return;
      }
    }
    ...
    if (lib.translate[name]) {
      ...
      if (Array.isArray(targets) && targets.length) {
        ...
        game.log(
          ...
          str,
          "发动了",
          "【" + get.skillTranslation(name, this) + "】"
        );
      } else {
        game.log(
          this,
  ...
  prompt_old(name2, className) {
    ...
    if (
      ...
      parseInt(this.dataset.position) ==
        parseInt(ui.arena.dataset.number) / 2 ||
      typeof name2 == "number" ||
      this.classList.contains("minskin")
    ) {
  ...
  initStorage(name, value, mark) {
    return this.hasStorage(name)
      ? this.getStorage(name)
      : this.setStorage(name, value, mark);
  }
  ...
  markCharacter(name, info, learn, learn2) {
    ...
    if (name.startsWith("unknown")) {
      ...
      ui.create.div(".background.skillmark", node).innerHTML =
        get.translation(name)[0];
    } else {
      if (!get.character(name)) {
        return;
      }
      node = ui.create
        .div(".card.mark.drawinghidden")
  ...
  canUse(card, target, distance, includecard) {
    ...
    return lib.filter[includecard ? "targetEnabledx" : "targetEnabled"](
      card,
      this,
      target
    );
  ...
  addSubPlayer(cfg) {
    ...
    const list = cfg.caption
      ? [cfg.caption]
      : ["", "_prefix", "_ab"].map((str) => lib.translate[cfg.name + str]);
    game.broadcastAll(
      ...
      function (player, skill, list, cfg) {
        ...
        lib.character[skill] = [
          ...
          parseFloat(cfg.hp) +
            "/" +
            parseFloat(cfg.maxHp) +
            "/" +
            parseFloat(cfg.hujia),
          cfg.skills,
          ["tempname:" + cfg.name].concat(
            lib.character[cfg.name].trashBin || []
          ),
        ];
  ...
  changeSkills(addSkill = [], removeSkill = [], popup = true) {
    if (!Array.isArray(addSkill) || !Array.isArray(removeSkill)) {
      console.warn(
        `警告：Player[${this.name}].changeSkills的参数错误，应当为数组形式。`
      );
      return;
    }
  ...
  removeAdditionalSkills(skill, target) {
    ...
    return player
      .changeSkills([], skills)
      .set("$handle", function (player, addSkills, removeSkills) {
  ...
  addTempSkills(skillsToAdd, expire) {
    ...
    return this.changeSkills(skillsToAdd, []).set(
      "$handle",
      function (player, addSkills, removeSkills) {
        if (addSkills.length) {
          game.log(
            player,
            "获得了技能",
          ...
          do {
            skillName =
              "player_tempSkills_" + Math.random().toString(36).slice(-8);
  ...
  clearSkills(all) {
    ...
    for (i = 0; i < this.skills.length; i++) {
      ...
      if (
        !all &&
        (lib.skill[this.skills[i]].temp || lib.skill[this.skills[i]].charlotte)
      ) {
        continue;
      }
    ...
    if (skill) {
      ...
      for (var i = 0; i < forbid.length; i++) {
        if (forbid[i][1] || this.name2) {
          this.forbiddenSkills[forbid[i][0]] =
            this.forbiddenSkills[forbid[i][0]] || [];
          if (forbid[i][1]) {
    ...
    if (mode == "identity") {
      ...
      if (player.identity == "ye") {
        ...
        targets = game[method](function (target) {
          ...
          return (
            target.identity == "ye" || lib.character[target.name1][1] != group
          );
        });
      }
  ...
  isMaxHp(only, raw) {
    return game.players.every((value) => {
      ...
      return only
        ? value.getHp(raw) < this.getHp(raw)
        : value.getHp(raw) <= this.getHp(raw);
    });
  }
  /**
  ...
  isMaxCard(only) {
    ...
    return game.players.every((value) => {
      ...
      return only
        ? value.countCards("he") < numberOfCards
        : value.countCards("he") <= numberOfCards;
    });
  }
  /**
  ...
  isUnseen(num) {
    switch (num) {
      ...
      case 2:
        return (
          this.classList.contains("unseen") ||
          this.classList.contains("unseen2")
        );
      default:
        return (
          this.classList.contains("unseen") &&
          (!this.name2 || this.classList.contains("unseen2"))
        );
    }
  }
  ...
  isMajor() {
    if (get.mode() == "guozhan") {
      ...
      var list = game.filterPlayer(function (current) {
        return (
          current.identity != "unknown" && current.hasSkillTag("forceMajor")
        );
      });
      if (list.length) {
        for (var i of list) {
  ...
  siege(player) {
    ...
    if (!player) {
      ...
      return (
        player.sieged() &&
        (player.getNext() == this || player.getPrevious() == this)
      );
    }
  }
  sieged(player) {
  ...
  checkShow(skill, showonly) {
    ...
    if (
      get.mode() != "guozhan" ||
      game.expandSkills(this.getSkills()).includes(skill)
    ) {
      if (showonly) {
        return false;
      } else {
  ...
  hasStockSkill(skill, arg1, arg2, arg3) {
    return game
      .expandSkills(this.getStockSkills(arg1, arg2, arg3))
      .includes(skill);
  }
  isZhu2() {
    var player = this,
  ...
  isInitFilter(tag) {
    ...
    for (const name of [player.name, player.name1, player.name2]) {
      if (name && lib.character[name]) {
        ...
        if (
          lib.characterInitFilter[name] &&
          lib.characterInitFilter[name](tag) === false
        ) {
          continue;
        }
        return true;
      ...
      if (info && info.ai) {
        ...
        if (info.ai[tag] === true) {
          if (typeof arg === "object" && arg && !info.ai.skillTagFilter) {
            console.log(
              `疑似忘给lib.skill.${skills[i]}.ai.${tag}加skillTagFilter了\nhasSkillTag：`,
              arg
            );
          }
          return true;
  ...
  hasWuxie(info) {
    ...
    for (var i = 0; i < skills.length; i++) {
      ...
      if (ifo.hiddenWuxie && info) {
        if (
          typeof ifo.hiddenWuxie == "function" &&
          ifo.hiddenWuxie(this, info)
        ) {
          return true;
        }
      } else if (
  ...
  mayHaveSha(viewer, type, ignore, rvt) {
    ...
    let hs = this.getCards("hs").filter(
      (i) => !cards.includes(i) && !selected.includes(i)
    ).length;
    if (!hs) {
      if (rvt === "count") {
        return count;
  ...
  $throwordered1(node, nosource) {
    ...
    for (var i = 0; i < ui.thrown.length; i++) {
      if (
        ui.thrown[i].parentNode != ui.arena ||
    ...
    if (nosource) {
      ...
      var parseCalc = function (str) {
        ...
        var add = str
          .slice(str.indexOf("%") + 1, str.indexOf("px"))
          .replace(/\s/g, "");
  ...
  $give(card, player, log, init, cardsetion) {
    ...
    if (get.itemtype(card) == "cards") {
      ...
      if (this.$givemod) {
        ...
        node.listenTransition(function () {
          var dx =
            player.getLeft() + player.offsetWidth / 2 - 52 - node.offsetLeft;
          var dy =
            player.getTop() + player.offsetHeight / 2 - 52 - node.offsetTop;
          if (
            node.style.transform &&
            node.style.transform != "none" &&
            node.style.transform.indexOf("translate") == -1
    ...
    cards.forEach((card) => {
      if (card.name.indexOf("empty_equip") == 0) {
        ...
        if ((num == 4 || num == 3) && get.is.mountCombined()) {
          remove =
            !player.hasEmptySlot("equip3_4") ||
            player.getEquips("equip3_4").length;
  ...
  addVirtualJudge(card, cards) {
    ...
    game.addVideo("addVirtualJudge", player, [
      get.vcardInfo(cardx),
      get.cardsInfo(cards),
    ]);
  ...
  $addVirtualJudge(VCard, cards) {
    ...
    const player = this,
      card = VCard;
    const isViewAsCard =
        cards.length !== 1 || cards[0].name !== VCard.name || !card.isCard,
    ...
    if (get.itemtype(card) == "card" && card.isViewAsCard) {
      ...
      cardx = isViewAsCard
        ? game.createCard(
            card.name,
            cards.length == 1 ? get.suit(cards[0]) : "none",
            cards.length == 1 ? get.number(cards[0]) : 0
          )
        : cards[0];
    }
    game.broadcastAll(
      (player, cardx, isViewAsCard, VCard, cards) => {
        ...
        if (isViewAsCard && !cardx.isViewAsCard) {
          ...
          cardx.destroyed = function (card, id, player, event) {
            ...
            if (
              id == "ordering" &&
              ["phaseJudge", "executeDelayCardEffect"].includes(
        ...
        if (isViewAsCard) {
          ...
          if (
            cardx.classList.contains("fullskin") ||
            cardx.classList.contains("fullborder") ||
            cardx.classList.contains("fullimage")
          ) {
            ...
            cardx.node.background.innerHTML =
              lib.translate[cardx.viewAs + "_bg"] ||
              get.translation(cardx.viewAs)[0];
          }
        } else {
          delete cardx.viewAs;
  ...
  addVirtualEquip(card, cards) {
    ...
    game.addVideo("addVirtualEquip", player, [
      get.vcardInfo(cardx),
      get.cardsInfo(cards),
    ]);
  ...
  $fullscreenpop(str, nature, avatar, broadcast) {
    ...
    if (avatar) {
      ...
      node.firstChild.firstChild.style.backgroundImage =
  ...
  $damage(source) {
    ...
    if (source && source != this && lib.config.damage_shake) {
      ...
      if (source.getTop() == this.getTop()) {
        ...
        var ratio =
          (source.getLeft() - this.getLeft()) /
          (source.getTop() - this.getTop());
